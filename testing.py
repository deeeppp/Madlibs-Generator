...
class screen3(BoxLayout,Screen):
    def __init__(self, **kwargs):
        ...
        self.ii = 1
        if self.ii == 1:
            a = ['Enter an adjective', 'Enter a noun:', 'Enter a name:', 'Enter a adjective (describing powers):', 'Enter an verb with ing form:','enter a noun (with ing form):','enter a verb:','enter an adjective:','enter an adjective describing a wild creature: ']
            self.hints = a
        if self.ii == 2:
...
class screen5(BoxLayout,Screen):
    ...
    def crime(self,inputs):
        i = self.ii
        if i == 1:
            self.next_label.text=f"It was a dark and stormy night when {self.inputs[0]}, a notorious {self.inputs[1]} criminal, decided to pull off the heist of the century. {self.inputs[0]} had been planning the robbery for months, and they had gathered a team of {self.inputs[2]} accomplices to help them carry it out. The target was a {self.inputs[3]} museum, and they were after a priceless {self.inputs[4]} that had been on display for years. As the storm raged on outside, {self.inputs[0]} and their crew snuck into the museum, avoided the {self.inputs[5]} security systems, and made off with the {self.inputs[6]}. But as they were making their escape, they heard the sound of sirens in the distance, and they knew they were in trouble."
            self.next_label.text_size = (self.next_label.width, None)
            self.next_label.size_hint_y = None
            self.next_label.height = self.next_label.texture_size[1]
        # these two stories are different
        if i == 2:
            self.next_label.text=f"Detective {self.inputs[0]} had seen a lot of {self.inputs[1]} crime scenes in their time, but this one was different. The victim was a {self.inputs[2]} millionaire who had been found dead in their {self.inputs[3]} under {self.inputs[4]} circumstances. As {self.inputs[0]} began to investigate the crime, they realized that the victim's {self.inputs[5]} was missing, and they suspected that the killer was after it. As they delved deeper into the case, {self.inputs[0]} uncovered a web of {self.inputs[6]} lies, {self.inputs[7]} secrets, and {self.inputs[8]} motives. But in the end, they were able to crack the case and bring the killer to justice."
            self.next_label.text_size = (self.next_label.width, None)
            self.next_label.size_hint_y = None
            self.next_label.height = self.next_label.texture_size[1]
        if i == 3:
            self.next_label.text=f"It was a sunny day when {self.inputs[0]}, a {self.inputs[1]} private investigator, received a new case. A wealthy client had reported that their {self.inputs[2]} had been stolen, and they wanted {self.inputs[0]} to find it. As {self.inputs[0]} began to investigate the case, they realized that the theft was just the tip of the iceberg. The client had been {self.inputs[3]} with some {self.inputs[4]} characters, and they had gotten involved in a {self.inputs[5]} scheme that could have serious consequences. But with their quick wit and {self.inputs[6]} detective skills, {self.inputs[0]} was able to solve the case and put an end to the scheme."
            self.next_label.text_size = (self.next_label.width, None)
            self.next_label.size_hint_y = None
            self.next_label.height = self.next_label.texture_size[1]
        if i == 4:
            self.next_label.text=f"In the heart of the city, a {self.inputs[0]} gang ruled the streets with an iron fist. The leader, known only as {self.inputs[1]}, was a {self.inputs[2]} mastermind who had built an empire on the profits of their {self.inputs[3]}. But their reign of terror came to an end when a new detective, {self.inputs[4]}, arrived on the scene. {self.inputs[4]} was determined to take down the gang and put {self.inputs[1]} behind bars. They gathered evidence, interviewed witnesses, and slowly but surely, they began to dismantle the gang's operation. In the end, they were able to catch {self.inputs[1]} red-handed and bring them to justice."
            self.next_label.text_size = (self.next_label.width, None)
            self.next_label.size_hint_y = None
            self.next_label.height = self.next_label.texture_size[1]
        if i == 5:
